...
pub fn file_system_open_file(filename: &str, mode: u32) -> Result<u32, i32> {  
    ...
    if let Some(file_rc) = file {  
        ...
        file_rc.borrow_mut().opened = true;  
        return Ok(fd);  
    }  
...
pub fn file_system_write_to_file(fd: u32, data: &[u8], size: u32, offset: u32) -> Result<u32, i32> {  
    ...
    if file.size < (offset + size) {  
        ...
        if let Err(e) = expand_file_size(&mut file, offset + size) {  
            eprintln!("Error: file_system_write_to_file: failed to expand file size: {:?}", e);  
...
pub fn initialize_file_system(partition_num_blocks: u32) {  
    ...
    DIR_DATA.with(|dir_data_o| {  
        ...
        if dir_data[0] == b'$' && dir_data[1] == b'%' && dir_data[2] == b'^' && dir_data[3] == b'&' {  
            ...
            drop(dir_data);
            let mut dir_data = dir_data_o.borrow_mut();  
            dir_data[0] = b'$';  
            dir_data[1] = b'%';  
            dir_data[2] = b'^';  
            dir_data[3] = b'&';  
...
fn expand_existing_file(file: &mut File, needed_blocks: u32) -> Result<(), i32> {  
    let mut found = true;  
...
fn expand_empty_file(file: &mut File, needed_blocks: u32) -> Result<(), i32> {  
    ...
    let zero_buf = [0u8; STORAGE_BLOCK_SIZE];  
    for i in 0..needed_blocks {  
        write_blocks(&zero_buf, start_block + i, 1).map_err(|_| ERR_MEMORY)?;  
    }  
...
fn expand_file_size(file: &mut File, size: u32) -> Result<(), i32> {  
    if file.size >= size {  
        return Ok(());  
    }  
    ...
    let ret = if empty_file {  
        expand_empty_file(file, needed_blocks)  
    } else {  
        expand_existing_file(file, needed_blocks)  
    };  
